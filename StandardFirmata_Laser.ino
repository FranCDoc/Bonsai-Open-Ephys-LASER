    ...
    for (pin = port * 8; pin < lastPin; pin++) {
      ...
      if (pin == 6) { // Through this pin binary information enters.
        if (digitalRead(6) == HIGH) { // If its a 1 (because there is activity in ROI) then a GREEN LED inserted in this pin is lit.
          ...
          digitalWrite(13, HIGH); // Laser is triggered.
          delay(5); // 5 ms long
          digitalWrite(13, LOW); // LASER OFF
          delay(0.05); // 0.05 ms long
        } else { // If its a 0 (because there is no activity in ROI) then GREEN LED will be in an OFF state.
          digitalWrite(13, LOW);// Laser is kept off.
